...
export default function LoginPage() {
  ...
  async function handlePasswordReset(e) {
    ...
    setResetMessage('')
    if (!resetEmail.trim()) {
      setResetError('E-posta adresi gerekli')
      return
    }
    ...
    try {
      ...
      setResetError(messages[err.code] || 'Bir hata oluştu. Lütfen tekrar deneyin.')
    }
    setResetLoading(false)
  }

  const inputClass = 'w-full px-4 py-3 rounded-xl border border-border-light dark:border-dark-border bg-surface-alt/50 dark:bg-dark-elevated dark:text-dark-text focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none text-sm transition-all'
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-gradient-mesh">
        <div className="w-full max-w-md animate-fade-in">
          ...
          <div className="hidden lg:block mb-8">
            <h1 className="font-heading text-3xl font-bold text-text dark:text-dark-text-heading">
              {showReset ? 'Şifre Sıfırla' : isRegister ? 'Hesap Oluştur' : 'Hoş Geldin!'}
            </h1>
          ...
          <p className="text-center text-text-muted mb-6 lg:hidden">
            {showReset ? 'Şifrenizi sıfırlayın' : isRegister ? 'Yeni hesap oluştur' : 'Hesabına giriş yap'}
          </p>

          <div className="bg-white dark:bg-dark-surface rounded-3xl border border-border-light dark:border-dark-border p-8 shadow-soft dark:shadow-dark-soft">
            {showReset ? (
              /* Password Reset Form */
              <div>
                <form onSubmit={handlePasswordReset} className="space-y-5">
                  ...
                  {resetMessage && (
                    <div className="flex items-center gap-2 text-sm text-secondary bg-secondary/10 px-4 py-3 rounded-xl">
                ...
                <button
                  ...
                  className="w-full text-center text-sm text-primary font-semibold hover:underline mt-5"
                >
                  {'\u2190'} Giriş sayfasına dön
                </button>
              </div>
              ...
              <>
                ...
                <div className="relative my-6">
                  ...
                  <div className="relative flex justify-center text-xs text-text-muted">
                    <span className="bg-white dark:bg-dark-surface px-4">veya</span>
                  </div>
                </div>

                <button
                  onClick={handleGoogle}
                  className="w-full flex items-center justify-center gap-3 px-4 py-3 border-2 border-border-light dark:border-dark-border rounded-xl hover:bg-surface-alt dark:hover:bg-dark-elevated hover:border-primary/20 text-sm font-semibold dark:text-dark-text transition-all duration-200 hover:shadow-soft"
                >
